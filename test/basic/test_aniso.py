...
def test_anisotropy():
    ...
    for pola in ["TE", "TM"]:
        ...
        atol = 1e-5
        assert np.allclose(wh, w, atol=atol)
        assert np.allclose(wh2, w, atol=atol)
        assert np.allclose(wh3, w, atol=atol)
        assert np.allclose(wh4, w, atol=atol)
        assert np.allclose(wh5, w, atol=atol)
