...
URL=$(shell python3 -c "import protis; print(protis.__website__)")
LESSC=$(PROJECT_DIR)/doc/node_modules/less/bin/lessc
GITLAB_PROJECT_ID=35660637
GITLAB_GROUP_ID=52441206
...
repo:
	$(call message,${@})
	xdg-open https://gitlab.com/protis/protis
...
checksum:
	$(call message,${@})
	@echo v$(VERSION)
	$(eval SHA256 := $(shell curl -sL https://gitlab.com/protis/protis/-/archive/v$(VERSION)/protis-v$(VERSION).tar.gz | openssl sha256 | cut  -c10-))
	@echo $(SHA256)
